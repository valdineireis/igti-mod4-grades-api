...
const findOne = async (req, res) => {
    ...
    try {
        ...
        if (data.length < 1) {
            res.status(404).send({
                message: 'Nenhuma Grade encontrada com o id:' + id,
            });
        } else {
            res.send(data);
        }
...
const update = async (req, res) => {
    ...
    const gradeAtualizada = { ...req.body, lastModified: new Date() };
    ...
    try {
        ...
        if (data.length < 1) {
            res.status(404).send({
                message: 'Nenhuma Grade encontrada para atualizar',
            });
        } else {
            res.send({ data, message: 'Grade atualizada com sucesso' });
        }
...
const remove = async (req, res) => {
    ...
    try {
        ...
        if (data.length < 1) {
            res.status(404).send({
                message: 'Nenhuma Grade encontrada para exclusao',
            });
        } else {
